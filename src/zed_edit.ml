...
type t = {
  ...
  changes : (Zed_rope.t * int * int) event;
  send_changes : (Zed_rope.t * int * int) -> unit;
...
let create ?(editable=fun pos -> true) ?(move=(+)) () =
  ...
  {
    text = S.hold ~eq:(==) Zed_rope.empty (E.map (fun (text, start, delta) -> text) changes);
    changes;
    send_changes;
    cursor_changes = E.map (fun (text, start, delta) -> (start, delta)) changes;
    editable;
    move;
  }
...
module Actions(Context : sig val context : context end) = struct
  ...
  let perform action =
    let new_text, new_position = action () in
    let new_length = Zed_rope.length new_text in
    ...
    let delta = new_length - length in
    if text == new_text then
      (* If the text has not changed, just move the cursor. *)
      Zed_cursor.move cursor delta
    else if delta >= 0 then begin
      edit.send_changes (new_text, start, delta);
      Zed_cursor.move cursor delta
    end else begin
      Zed_cursor.move cursor delta;
      edit.send_changes (new_text, start, delta);
