...
val check : t -> (int, string) Result.result
...
(** [next_error str ofs] returns [(ofs', count, msg)] where [ofs'] is the offset of the
    ...
    message. If there is no error until the end of string then [ofs] is [String.length
    str] and [msg] is the empty string. *)
...
(** [rev_init n f] returns the contenation of [singleton (f (n - 1))], ..., [singleton (f
    1)], [singleton (f 0)]. *)
...
(** [rev_filter_map f str] filters and maps characters of [str] with [f] in reverse
    order. *)
...
(** [escaped_string enc str] is the same as [escaped] except that [str] may contain
    invalid UTF-8 data. In this case invalid characters are interpreted as Latin-1. *)

val add_escaped_string : Buffer.t -> string -> unit
(** [add_escaped_string buf enc text] is the same as [Buffer.add_string buf
    (escaped_string enc text)] but a bit more efficient. *)
